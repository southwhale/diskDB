...
export default class Collection {
  ...
  save(data, useAutoIncrementId = false) {
    var collection = this._parse();
    if (typeof data === 'object' && data.length) {
      ...
      for (var i = data.length - 1; i >= 0; i--) {
        var d = data[i];
        d._id = useAutoIncrementId ? this.count() + 1 : UUID().replace(/-/g, '');
        collection.push(d);
      ...
      data._id = useAutoIncrementId ? this.count() + 1 : UUID().replace(/-/g, '');
  ...
  update(query, data, options) {
    ...
    if (records.length) {
      ...
      if (options && options.upsert) {
        data._id = options.useAutoIncrementId ? this.count() + 1 : UUID().replace(/-/g, '');
        collection.push(data);
        ret.updated = 0;
        ret.inserted = 1;
      } else {
