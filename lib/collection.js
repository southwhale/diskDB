    ...
    coltn.save = function(data, autoIncreasement = false) {
        ...
        if (typeof data === 'object' && data.length) {
            ...
            for (var i = data.length - 1; i >= 0; i--) {
                var d = data[i];
                d._id = autoIncreasement ? coltn.count() + 1 : uuid.v4().replace(/-/g, '');
                collection.push(d);
            ...
            data._id = autoIncreasement ? coltn.count() + 1 : uuid.v4().replace(/-/g, '');
    ...
    coltn.update = function(query, data, options) {
        ...
        if (records.length) {
            ...
            if (options && options.upsert) {
                data._id = options.autoIncreasement ? coltn.count() + 1 : uuid.v4().replace(/-/g, '');
                collection.push(data);
                ret.updated = 0;
                ret.inserted = 1;
            } else {
